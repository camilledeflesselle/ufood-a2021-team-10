...
export const createListFavorites = async (listName, userEmail) => {
  ...
  myHeaders.append(
    "Cookie",
    "connect.sid=s%3A75zFyvTZRk7rQ_juDpjNfzmZvkZbe2pe.xaSZI%2BUpGP6nYVCbxbTqoTf%2B6EpxHIFaF%2BKs0VATF3Q"
  );
...
export const getListFavorites = async (id) => {
  ...
  const request = new Request(`${res.url}/users/${id}/favorites`, {
    method: "GET",
...
export const updateListFavorites = async ({ owner, id, name }, token) => {
  let myHeaders = new Headers();
  myHeaders.append("Content-Type", "application/x-www-form-urlencoded");
  myHeaders.append(
    "Cookie",
  ...
  const request = new Request(`${res.url}/favorites/${id}`, requestOptions);
  const response = await fetch(request);
//   const request = new Request(`${endpoint2}/favorites/${id}`, {
//     method: "PUT",
...
export const deleteListFavorites = async (id) => {
  const res = testToken(new Headers());
  const request = new Request(`${res.url}/favorites/${id}`, {
    method: "DELETE",
